...
void free_btree(btree_node *node){
	if(node!=NULL){
		if(node->key!=NULL){
			free(node->key);
			node->key =NULL;
...
btree_node* makeNode(int order){
	...
	//This fix was done inorder to accomodate the some weird padding related problem
	if(order%2==0){
		temp->child = (long *)calloc(order,sizeof(long));
	} else {
		temp->child = (long *)calloc(order+1,sizeof(long));
	}
	return (temp);
}
...
long bsearch_tree(int target,long offset,int order,FILE *fp){
	...
	while(s < root->n) {
		if(target == root->key[s]){
			long ret_child = root->child[s];
			free_btree(root);		//this is to make sure the cleanup of memory
			return ret_child;
	...
	if(root->child[s]!=0){
		long ret_val = bsearch_tree(target,root->child[s],order,fp);
		free_btree(root);
		return ret_val;
...
void printBTree(FILE *fp,int order,long offset){
	...
	while(1){
		...
		while (qSize > 0){
			...
			free_btree(node);	//this free is written as next call will don new allocation
